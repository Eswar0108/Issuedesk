...
class OllamaProvider(BaseLLMProvider):
    ...
    def client(self) -> OpenAI:
        """Get the OpenAI client configured for local Ollama instance."""
    ...
    def is_configured(self) -> bool:
        """Check if local Ollama server is running by attempting to connect to port 11434."""
        try:
    ...
    def _ensure_configured(self):
        if not self.is_configured():
            raise HTTPException(
                ...
                detail=(
                    "Local Ollama server is not running on http://localhost:11434. "
                    "To use offline free models:\n"
                    "1. Download and start Ollama from https://ollama.com/\n"
                    "2. Open a terminal and download models:\n"
                    "   ollama pull llama3\n"
                    "   ollama pull nomic-embed-text\n"
                    "3. Make sure Ollama app is running."
                ),
            )
